use crate::runtime;
use crate::sidetree::payload::PublicKeyPayload;
use crate::{errors::NodeXDidCommError, runtime::base64_url::PaddingType};
use hex;
use ibig::{ibig, IBig};
...
impl Secp256k1 {
    ...
    pub fn new(context: &Secp256k1Context) -> Result<Self, NodeXDidCommError> {
    ...
    pub fn from_jwk(jwk: &KeyPairSecp256K1) -> Result<Self, NodeXDidCommError> {
    ...
    pub fn to_jwk(
        &self,
        included_private_key: bool,
    ) -> Result<KeyPairSecp256K1, NodeXDidCommError> {
    ...
    pub fn to_public_key(
        ...
        key_id: &str,
        purpose: &[&str],
    ) -> Result<PublicKeyPayload, NodeXDidCommError> {
        let validated = match self.validate_point() {
            Ok(v) => v,
            Err(e) => {
                log::error!("{:?}", e);
    ...
    pub fn get_point_x(&self) -> Result<Vec<u8>, NodeXDidCommError> {
    ...
    pub fn get_point_y(&self) -> Result<Vec<u8>, NodeXDidCommError> {
        let public = self.get_public_key();
    ...
    pub fn validate_point(&self) -> Result<bool, NodeXDidCommError> {
        let x = match self.get_point_x() {
            Ok(v) => v,
            Err(e) => {
                log::error!("{:?}", e);
    ...
    pub fn transform_uncompressed_public_key(
        compressed: &[u8],
