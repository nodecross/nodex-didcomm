...
use crate::{
    config::did::KeyPair,
    config::{did_config, SingletonDidConfig},
...
impl KeyPairing {
    ...
    pub fn load_keyring() -> Result<Self, NodeXDidCommError> {
    ...
    pub fn create_keyring() -> Result<Self, NodeXDidCommError> {
        let config = did_config();
        let secure_keystore = SecureKeyStore::new();
    ...
    pub fn generate_secp256k1(
        seed: &[u8],
        derivation_path: &str,
    ) -> Result<Secp256k1, NodeXDidCommError> {
    ...
    pub fn get_identifier(&self) -> Result<String, NodeXDidCommError> {
        let did = self.config.inner.lock().unwrap().get_did();

        match did {
            Some(v) => Ok(v),
            None => Err(NodeXDidCommError {}),
        }
    }
}
