...
use crate::nodex::keyring::secp256k1::{COMPRESSED_PUBLIC_KEY_SIZE, PRIVATE_KEY_SIZE};
...
impl BIP32 {
    pub fn get_node(seed: &[u8], derivation_path: &str) -> Result<BIP32Container, BIP32Error> {
        ...
        Ok(BIP32Container {
            private_key: private_key.private_key.serialize_secret(),
            public_key: public_key.public_key.serialize(),
        })
    }
}
...
pub mod tests {
    ...
    #[test]
    fn test_get_node() {
        let result = BIP32::get_node(&seed(), &derivation_path()).unwrap();
